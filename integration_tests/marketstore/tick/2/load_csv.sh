...
mkts -rootDir /project/data/mktsdb <<EOF
\load TEST/1Min /project/tick/2/ticks-example.csv /project/tick/2/ticks-example.yaml
\create TEST/1H/TICK:Symbol/Timeframe/AttributeGroup bid,ask/float32 variable
\load TEST/1H/TICK /project/tick/2/ticks-example.csv /project/tick/2/ticks-example.yaml
EOF
